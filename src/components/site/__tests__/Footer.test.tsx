...
describe("Footer", () => {
  ...
  it("renderiza sem lançar 'Objects are not valid as a React child'", () => {
    expect(() => renderWithRouter(<Footer />)).not.toThrow();
  });

  it("não renderiza nenhum [object Object] no DOM", () => {
    const { container } = renderWithRouter(<Footer />);
    expect(container.textContent).not.toContain("[object Object]");
    expect(container.textContent).not.toContain("{label");
  });

  it("todos os links possuem href não vazio e clicável", () => {
    ...
    for (const link of links) {
      ...
      expect(href).not.toContain("$"); // sem rotas dinâmicas não resolvidas
    }
  });

  it("renderiza colunas e itens conhecidos", () => {
    renderWithRouter(<Footer />);

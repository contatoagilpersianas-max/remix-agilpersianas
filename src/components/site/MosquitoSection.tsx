...
import { MOSQUITO_DEFAULTS, type MosquitoConfig } from "@/components/admin/site/MosquitoModule";
import { useSiteContact } from "@/hooks/use-site-contact";

export function MosquitoSection() {
  const { value: cfg } = useSiteSetting<MosquitoConfig>("mosquito", MOSQUITO_DEFAULTS);
  const contact = useSiteContact();
  if (!cfg.enabled) return null;
  const phone = (contact.whatsapp || "5532351202810").replace(/\D/g, "");
  const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(cfg.ctaWhatsappMessage)}`;
  return (
    <section className="bg-sand py-12 md:py-16">
      <div className="container-premium grid gap-12 md:grid-cols-2 md:items-center md:gap-16">
        <div>
          ...
          >
            {cfg.titleLine1} <br />
            <span style={{ color: "#F57C00" }}>{cfg.titleLine2}</span>
          </h2>
